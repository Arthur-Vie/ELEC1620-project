...
The UART serial communication with the PC is relatively slow and there is quite a lot of data that needs to be sent, angle in 3 axes and position in 3 axes. To send all of the data for each sampling interval in full would take many tens of milliseconds, making the integration loop much too slow.
...
embedded below are some animations demonstrating the functionality so far:

![gif](Documentation/raw_data.gif)
...
- ### Calculating accelerations in world space
    ...
    Thankfully if sufficiently accurate and repeatable values for the device's orientation can be generated then this may be possible. If the device is assumed to start in a known orientation relative to the global coordinate system then the change in orientation over time can be used to update a transform that will convert the acceleration readings from the local to the global coordinates system.
